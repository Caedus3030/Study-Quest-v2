...
struct ContentView: View {
    ...
    @State private var avatarEyes = AvatarEyes.happy
    @State private var avatarMouth = AvatarMouth.smile
    ...
    private var mainAppView: some View {
        TabView(selection: $selectedTab) {
            ...
            AvatarStudioView(
                ...
                selectedEyes: $avatarEyes,
                selectedMouth: $avatarMouth
            )
            .tabItem {
...
private struct LaunchScreenView: View {
    ...
    let avatarEyes: AvatarEyes
    let avatarMouth: AvatarMouth
...
private struct AvatarStudioView: View {
    ...
    @Binding var selectedEyes: AvatarEyes
    @Binding var selectedMouth: AvatarMouth
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 22) {
                    VStack(spacing: 14) {
                        AvatarPreview(color: selectedColor.color, accessory: selectedAccessory, eyes: selectedEyes, mouth: selectedMouth)
                    ...
                    CustomizerSection(title: "Eyes") {
                        LazyVGrid(columns: [GridItem(.adaptive(minimum: 104), spacing: 12)], spacing: 12) {
                            ForEach(AvatarEyes.allCases) { eyes in
                                EyesChoiceButton(
                                    eyes: eyes,
                                    isSelected: selectedEyes == eyes,
                                    action: { selectedEyes = eyes }
                                )
                            }
                        }
                    }

                    CustomizerSection(title: "Mouth") {
                        LazyVGrid(columns: [GridItem(.adaptive(minimum: 104), spacing: 12)], spacing: 12) {
                            ForEach(AvatarMouth.allCases) { mouth in
                                MouthChoiceButton(
                                    mouth: mouth,
                                    isSelected: selectedMouth == mouth,
                                    action: { selectedMouth = mouth }
                                )
                            }
                        }
                    }
...
private struct EyesChoiceButton: View {
    let eyes: AvatarEyes
    let isSelected: Bool
    let action: () -> Void
    ...
    var body: some View {
        Button(action: action) {
            VStack(spacing: 8) {
                Image(systemName: eyes.iconName)
                    .font(.title2.weight(.bold))
...
private struct MouthChoiceButton: View {
    ...
    var body: some View {
        Button(action: action) {
            VStack(spacing: 8) {
                Image(systemName: mouth.iconName)
                    .font(.title2.weight(.bold))
                    .foregroundStyle(isSelected ? Color.white : Color.primary)
                    .frame(width: 50, height: 50)
                    .background(isSelected ? Color.primary : Color.primary.opacity(0.12))
...
private struct AvatarPreview: View {
    ...
    private var accessoryImageOffset: CGFloat {
        switch accessory {
        case .stars:
            return -18
        case .goggles:
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ZStack(alignment: .top) {
                    ...
                    Image(systemName: accessory.iconName)
                        .font(accessory == .goggles ? .system(size: 44, weight: .bold) : .title.weight(.bold))
                        .foregroundStyle(accessory.color)
                        .offset(y: accessoryImageOffset)

                    AvatarFaceView(eyes: eyes, mouth: mouth)
                        .offset(y: 48)
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 16) {
                    HStack(spacing: 16) {
                        AvatarPreview(color: avatarColor.color, accessory: avatarAccessory, eyes: avatarEyes, mouth: avatarMouth)
...
private struct AvatarFaceView: View {
    let eyes: AvatarEyes
    let mouth: AvatarMouth
...
private struct SadMouthShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let lineWidth = max(3, rect.height * 0.3)
        let insetRect = rect.insetBy(dx: lineWidth / 2, dy: lineWidth / 2)

        path.move(to: CGPoint(x: insetRect.minX, y: insetRect.minY + insetRect.height * 0.72))
        path.addQuadCurve(
            to: CGPoint(x: insetRect.maxX, y: insetRect.midY),
            control: CGPoint(x: insetRect.midX, y: insetRect.minY)
        )
...
private struct OpenSmileShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let lineWidth = max(3, rect.height * 0.28)
        let insetRect = rect.insetBy(dx: lineWidth / 2, dy: lineWidth / 2)

        path.addArc(
            center: CGPoint(x: insetRect.midX, y: insetRect.minY),
...
private enum AvatarEyes: String, CaseIterable, Identifiable {
    case happy = "Happy"
    case sleepy = "Sleepy"
    case casual = "Casual"
    case cool = "Cool"
...
private enum AvatarMouth: String, CaseIterable, Identifiable {
    case smile = "Smile"
